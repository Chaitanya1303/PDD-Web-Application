...
describe('Glowtics Selenium E2E suite', function () {
  ...
  it('Analyze should show the remaining steps warning before analysis is ready', async () => {
    await seedLocalStorage(driver, { user: { name: 'Glow User', memberSince: 'June 2026' }, analysisResults: {} });
    await driver.navigate().refresh();
    await clickByText(driver, 'button', 'Analyze');
  ...
  it('Reports should display a no reports prompt when no scan exists', async () => {
    await seedLocalStorage(driver, { user: { name: 'Glow User', memberSince: 'June 2026' }, analysisResults: {} });
    await driver.navigate().refresh();
  ...
  it('Nutrition should display a no nutrition plan prompt when no scan exists', async () => {
    await seedLocalStorage(driver, { user: { name: 'Glow User', memberSince: 'June 2026' }, analysisResults: {} });
    await driver.navigate().refresh();
  ...
  it('Profile should render profile details and logout button', async () => {
    await seedLocalStorage(driver, { user: { name: 'Glow User', memberSince: 'June 2026' }, analysisResults: {} });
    await driver.navigate().refresh();
  ...
  it('Profile should allow editing the user name successfully', async () => {
    await seedLocalStorage(driver, { user: { name: 'Glow User', memberSince: 'June 2026' }, analysisResults: {} });
    await driver.navigate().refresh();
    await clickByText(driver, 'button', 'Profile');
  ...
  it('Profile should allow adding a product to the current products list', async () => {
    await seedLocalStorage(driver, { user: { name: 'Glow User', memberSince: 'June 2026' }, analysisResults: {} });
    await driver.navigate().refresh();
    await clickByText(driver, 'button', 'Profile');
  ...
  it('Profile should preserve the user name across refresh via localStorage', async () => {
    await seedLocalStorage(driver, { user: { name: 'Persisted User', memberSince: 'June 2026' }, analysisResults: {} });
    await driver.navigate().refresh();
    await clickByText(driver, 'button', 'Profile');
    const persisted = await driver.findElement(By.xpath('//h2[text()="Persisted User"]'));
    expect(await persisted.isDisplayed()).to.be.true;
  });

  it('Home should show the no activity placeholder before scans', async () => {
    await seedLocalStorage(driver, { user: { name: 'Glow User', memberSince: 'June 2026' }, analysisResults: {} });
    await driver.navigate().refresh();
  ...
  mealSections.forEach((section) => {
    it(`Nutrition should add a meal log item for ${section}`, async () => {
      const state = {
      ...
      const input = await driver.findElement(By.xpath(`//div[.//div[text()="${section}"]]//input`));
      await input.sendKeys(`Item ${section}`);
      await input.sendKeys('\uE007');
      const added = await driver.findElement(By.xpath(`//div[contains(text(),"Item ${section}")]`));
      expect(await added.isDisplayed()).to.be.true;
    });
  });

  mealSections.forEach((section) => {
    it(`Nutrition should remove a meal log item for ${section}`, async () => {
      const state = {
        user: { name: 'Glow User', memberSince: 'June 2026' },
        analysisResults: {
          skinAnalysis: {
      ...
      await clickByText(driver, 'button', '+');
      const input = await driver.findElement(By.xpath(`//div[.//div[text()="${section}"]]//input`));
      await input.sendKeys(`Remove ${section}`);
      await input.sendKeys('\uE007');
      const added = await driver.findElement(By.xpath(`//div[contains(text(),"Remove ${section}")]`));
      const removeBtn = await added.findElement(By.xpath('.//button'));
      await removeBtn.click();
      const removed = await driver.findElements(By.xpath(`//div[contains(text(),"Remove ${section}")]`));
      expect(removed.length).to.equal(0);
    });
  });

  it('Nutrition should enable the AI insights button when a plan exists', async () => {
    const state = {
      user: { name: 'Glow User', memberSince: 'June 2026' },
      analysisResults: {
        skinAnalysis: {
  ...
  it('Reports should render trigger analysis bars when data exists', async () => {
    const state = {
      user: { name: 'Glow User', memberSince: 'June 2026' },
  ...
  it('Profile should reset to onboarding when logout confirm is accepted', async () => {
    await seedLocalStorage(driver, { user: { name: 'Glow User', memberSince: 'June 2026' }, analysisResults: {} });
    await driver.navigate().refresh();
    await clickByText(driver, 'button', 'Profile');
  ...
  reportFilters.forEach((filter) => {
    it(`Reports should change filter to ${filter}`, async () => {
      const state = {
        user: { name: 'Glow User', memberSince: 'June 2026' },
  ...
  for (let index = 1; index <= 12; index++) {
    it(`Sanity smoke test refresh #${index}`, async () => {
      await seedLocalStorage(driver, { user: { name: 'Glow User', memberSince: 'June 2026' }, analysisResults: {} });
      await driver.navigate().refresh();
      await driver.wait(until.elementLocated(By.xpath('//button[contains(.,"Home")]')), 10000);
